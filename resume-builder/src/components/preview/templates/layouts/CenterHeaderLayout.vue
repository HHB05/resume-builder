<template>
  <div class="bg-white p-8">
    <!-- Center Header -->
    <div class="text-center mb-6 pb-4" :style="{ borderBottom: `3px solid ${config.primaryColor}` }">
    ...
    <div v-if="content.education?.length" class="mb-6">
      <h2 class="text-lg font-bold text-center mb-3 pb-2" :style="{ color: config.primaryColor, borderBottom: `2px solid ${config.primaryColor}30` }">
        教育背景
      </h2>
    ...
    <div v-if="content.summary">
      ...
      <p class="text-sm text-gray-600 leading-relaxed text-center">{{ content.summary }}</p>
    </div>
  </div>
</template>
